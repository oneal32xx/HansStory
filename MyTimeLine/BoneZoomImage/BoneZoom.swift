...
class BoneZoom: UIView {
    ...
    convenience init(imageView: UIImageView) {
        ...
        self.navView.titleLabel.text = "預覽"
        self.addSubview(self.scrollView)

        self.newImage = UIImageView(image: imageView.image)
        self.newImage.contentMode = .scaleAspectFill
    ...
    @objc private func saveAction() {
        let item: [MMPopupItem] = [MMItemMake("儲存圖片到相簿", .highlight, { (index) in
            if let image = self.newImage.image {
                UIImageWriteToSavedPhotosAlbum(image, self, #selector(self.image(image:didFinishSavingWithError:contextInfo:)), nil)
            }
        })]
        BoneShowHUD.shared.sheet(title: "目前動作", items: item)
    }
    ...
    @objc private func image(image: UIImage, didFinishSavingWithError error: NSError?, contextInfo:AnyObject) {
        ...
        if error != nil {
            BoneShowHUD.shared.show(status: "儲存圖片失敗", showType: .Error)
        } else {
            BoneShowHUD.shared.show(status: "儲存圖片成功", showType: .Info)
        }
    }
}
